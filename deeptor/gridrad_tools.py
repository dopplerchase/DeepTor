class gridrad:
    ...
    def to_xarray(self, key_list = ['Z_H','Z_DR','K_DP','r_HV']):
        for i in key_list:
            self._append_data_array(variable_key=i)
            
    def _append_data_array(self,variable_key='Z_H'):
        ...
        import xarray as xr 
        da = xr.DataArray(data[variable_key]['values'], 
                          dims=['alts','lats','lons'],
                          coords={'lons': data['x']['values'],
                                  'lats': data['y']['values'],
                                  'alts': data['z']['values']})
        da.fillna(value=-9999)
        
        if self.ds is None:
            self.ds = da.to_dataset(name = variable_key)
        else:
            self.ds[variable_key] = da 
